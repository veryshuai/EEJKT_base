function [exit_by_age,max_mat_age,brooks] =  match_summary_v2(agg_mat_yr_sales,agg_match_count,mm);
...
% theta_ndx = pt_type(pt_ndx,2);
 ...
 %   agg_mat_yr_sales: [t,type,firm ID, match sales, shipments, boy Z, eoy Z, match age, firm age] 
     agg_mat_yr_sales(:,1) = floor(agg_mat_yr_sales(:,1)/mm.pd_per_yr); % create integer year variable
...
%        exit_rate(aa,qq) = (sum(matdat_qs(gg,1)==0) + sum(1-theta_qq(gg)))/sum(matdat_qm(gg,1),1);
       exit_rate(aa,qq) = (sum((matdat_qs(gg,1)==0).*(sales_qq(gg)>0)) + sum(1-theta_qq(gg)))/sum(matdat_qm(gg,1),1);
