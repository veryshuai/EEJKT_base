% This function simulates panel data on foreign sales, matches, and shipments for all firms of a
% particular type. Type is determined by productivity and foreign theta.

function [singletons, agg_time_gaps,agg_mat_yr_sales,agg_firm_yr_sales,...
          ...
          agg_mat_exit_x,agg_mat_exit_y,agg_mat_matur,...
          agg_mat_obs,agg_nmat_exit,agg_ship_obs,agg_ln_ships,agg_match_count,my_flag]...
    = matchdat_gen_f3(N_firms,N_types,N_Z,N_Phi,N_mic_state,pd_per_yr,frac_of_year,periods,...
   ...
   for i=1:N_firms
    % break down new clients that occur between t-1 and t into e.o.p. z-types 
      ...
      if sum(new_cli_zst(i,:)) ~= add_cli_cnt(i,t) % cheap patch--better to clean up C++ code
       'C routine for random draws failed in matchdat_gen_f3. Trying Matlab version'
...
%% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
% Calculate time in export market  
  flr       = max(flrlag,t*new_firm(:,t)); % floor resets to current year for new exporters. Hence
...
%  Load season to season transitions into mat_tran, which describes  
...
% convert age in periods to age in years. The figure for mat_yr_sales will be 
% over-written after burn-in years in mat_yr_spice
%  mat_yr_sales(:,6:7) = floor(mat_yr_sales(:,6:7)./pd_per_yr) + ones(size(mat_yr_sales,1),2);
%   firm_yr_sales(:,4) = floor(firm_yr_sales(:,4)./pd_per_yr) + ones(size(firm_yr_sales,1),1);
         

% mat_yr_sales: [firm ID, match-specific sales, shipments, boy Z, eoy Z, match age w/in yr, firm age] 
% firm_yr_sales:[firm ID, total exports, total shipments, firm age]

% # unsuccessful meetings (duds) over previous year, by firm (needed for degree distribution later)
        yr_lag = t-pd_per_yr;
        cum_duds  = cum_meets(:,yr_lag:t) - cum_succ(:,yr_lag:t); % previous pd_per_yr + 1 cumulative duds
        curr_duds = cum_duds(:,2:pd_per_yr+1)-(new_firm(:,yr_lag+1:t)==0).*cum_duds(:,1:pd_per_yr);
...
%    
%           tt =  ones(size(mat_yr_sales,1),1).*[t,mic_type];
%           agg_mat_yr_sales  = [agg_mat_yr_sales;[tt,mat_yr_sales]];
%        % agg_mat_yr_sales: [t,type,firm ID, match sales, shipments, boy Z, eoy Z, w/in yr. match age, w/in year firm age] 
...
%% Construct and cumulate moments       
  ...
  if year > mm.burn  % construct moments for match regressions, firm export regressions   
...
%% the following matrices accumulate annualized values over time and firm types
 ...
 % mat_yr_splice splices consecutive obs. on annualized data and cumulates 
 % match ages (measured in periods). It's really costly! (8 seconds)  
 
    ncols = size(mat_yr_sales,2);  
    [mat_cont_2yr,mat_yr_sales] = mat_yr_splice(mat_yr_sales,mat_yr_sales_lag,ncols,mm);
        ...
        assert(sum(mat_cont_2yr(:,14)- mat_cont_2yr(:,13)<0)==0) % firm not less than match age
          catch
       'pause here'
...
%  match age and firm age are updated from w/in year (in periods) to totals (in years) 
%  before mat_yr_sales is passed back from mat_yr_splice
...
%       size([mat_yr_sales(:,2),mat_yr_sales(:,4:7),mat_yr_sales(:,1)])
%       size(ones(size(mat_yr_sales,1)).*t/pd_per_yr)
%       size(agg_mat_matur)
   ...
   %  match regression moments
    ...
    % match exit regression moments
      ...
      [mat_exit_x,mat_exit_y,mat_exit_moms_xx,mat_exit_moms_xy,mat_obs,nmat_exit]...
            = match_exit_moms(mat_yr_sales,pd_per_yr);
...
%           x4 = log(1+matches(ff,7));            % age of exporter
    ...
    % shipment and match counter
      ...
      [nship_obs,ln_ships,match_count] = match_shpt_cntr(mat_yr_sales,max_match);
