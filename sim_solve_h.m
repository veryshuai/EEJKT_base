...
%search and learning Colombia Project.
    ...
    l = max((max(a*(pi + V_succ-x),0)*(1 + log(n))^gam*scl/cscale).^bet-1,0);
    ...
    % Calculate jacobian
    dl = (-a*bet*(1 + log(n))^gam*(a*(pi+V_succ-x)*(1 + log(n))^gam*scl/cscale).^(bet-1)).*(l>0);
    dc = (1+l).^(1/bet)/((1 + log(n))^gam*scl/cscale) .* dl;
