...
% Executed by distance.m, this script calls the functions needed to
% calculate the value functions and transition probabilites for firm in
% different states. The key calls are to inten_sim_v1.m (which calculates
% the policy functions) and Q2pmat_v2 (which uses the policy functions to 
% calculate intensity matrices and imputes discrete time transition
% matrices from them).
...
pmat_msf = expm(frac_of_year.*mm.Q_f); % JT: added this (foreign macro state transitions)
% Q_f based on annual data, so need frac_of_year
pmat_msf = pmat_msf./sum(pmat_msf,2);
pmat_cum_msf = cumsum(pmat_msf')';   

pmat_msh = expm(frac_of_year.*mm.Q_h);
...
% overall_client_dist_type = cell(size(typemat,1),1);
%tic
