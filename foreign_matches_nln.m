% This script snippet creates no learning foreign matches for a single firm
...
for n = 1:size(deathind,1)
    ...
    for k = lag:deathind(n)
        if ind(k-1,1) < shock_year % before macro policy change
            lam = lambda_f_orig(1,succ_prob(obin_fix(j),3),min(s,net_size)+1,ind(k-1,2),ind(k-1,4));
            exp_inv_temp = log(rand)/-lam;
        else
            lam = lambda_f_new(1,succ_prob(obin_fix(j),3),min(s,net_size)+1,ind(k-1,2),ind(k-1,4));
            exp_inv_temp = log(rand)/-lam;
        end
