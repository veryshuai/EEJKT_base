...
%       b_hazDAT   = [-0.3258+log(12),-0.8181,0.3117,-1.1323,2.4514,-0.7082]; % from match_lag_coefsDAT (and RDC release)
        ...
        for i = 1:grdsize+1  % cum success 
            for j = 1: grdsize+1  % success rate 
                ...
                mod_mkt_exit(i,j) = beta_mkt_exit(1) + beta_mkt_exit(2)*csucc_grid(i) ...
                   + beta_mkt_exit(3)*csucc_grid(i)^2 + beta_mkt_exit(4)*succ_grid(j)...
                    +beta_mkt_exit(5)*succ_grid(j)^2 + beta_mkt_exit(6)*csucc_grid(i)*succ_grid(j)   ;
                ...
                mod_haz(i,j) =  b_haz(1) + b_haz(2)*csucc_grid(i) ...
                   +b_haz(3)*csucc_grid(i)^2 + b_haz(4)*succ_grid(j)...
                   +b_haz(5)*succ_grid(j)^2  + b_haz(6)*csucc_grid(i)*succ_grid(j)   ;
            end
        end
