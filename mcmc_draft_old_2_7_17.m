...
%   Detailed explanation goes here
    ...
    % lnF         =  scale_h+log(X(1));     fixed costs per match 
    % delta       =  X(2);                  exog. match separation
    % scale_h     =  X(3);                  home profit function scalar
    % scale_f     =  scale_h + log(X(4));   foreign profit function scalar
    % beta        =  X(5);                  cost function convexity
    % a           =  X(6)*X(7);             beta dist. success parameter
    % b           =  X(7)*(1-X(6));         beta dist. failure parameter
    ...
    % cost scalar =  X(12);                 cost function scalar  
    % sig p       =  X(13);                 Pareto parameter

%% set priors (all uniform for now)

 X = [10.66651   0.50628   6.89762   1.88621   0.44484   0.36833  98.33169 ...
      1.78064   0.20929   4.49888  -2.97151 271.46013   0.34166];

% search cost function parameters: 

scst = [X(5),X(11),X(12)];
scst_lb = [0.001, -10, 0.1];
scst_ub = [100.0,  10, 500];  

% match flow payoff parameters:  

prof =[X(2),X(3),X(4),X(10)];
prof_lb = [  0.1,  0.1,  0.1,    0.1];  
prof_ub = [100.0, 100.0, 100.0, 100.0];

% match persistence parameters 

per =[X(1),X(8),X(9)]; 
per_lb = [  0.1,  0.005, 0.005];  
per_ub = [100.0,  100.0, 10.00];
