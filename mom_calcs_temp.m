...
exp_dom_coefs = [exp_dom_raw_coefs;mean(resids.^2)]; %we actually want the slope and mse only for this regression

exp_share = sale_f_mat./sale_h_mat;
[exp_share_coefs,resids] = regress_elim_nonreals(log(exp_share((sale_f_mat > 0) & (sale_h_mat > 0))),...
      ones(size(sale_f_mat((sale_f_mat > 0) & (sale_h_mat > 0)))),[],[],[],[]);
...
exp_frac_coefs = exporter_count / firm_count; %fraction of exporters
