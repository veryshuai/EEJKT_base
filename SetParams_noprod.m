...
% mm.F         = exp(lnF);
mm.F         = F;         % cost of maintaining a client--home or foreign
...
mm.pi_tolerance  = 1e-3;    % convergence tolerance, profit function (WAS .001)
...
% Here we generate 2 * mm.phi_size  quantiles of the generalized pareto with parameter "sig_p".
% We set the other two parameters such that the distribution is pareto and 
% the mean is equal to 1, according to the formula mean (pareto) = tail param / (tail param - 1) * min.
% because of the way matlab sets this up, we want 0 < sig_p < 1 in order to get a finite mean (of one)

% set the parameters
gp_sigma = sig_p - sig_p^2; %sig_p is the "tail parameter" generalized pareto.  gp_sigma is the "scale parameter"
gp_theta = gp_sigma / sig_p; %theta is the "location parameter of the generalized pareto.

% get the correct number of evenly spaced quantiles
erg_pp = ones(2 * mm.phi_size + 1,1) / (2 * mm.phi_size + 1); %equal probability of landing at each quantile
Phi = gpinv(cumsum(erg_pp) - 0.5 / (2 * mm.phi_size + 1), sig_p, gp_sigma, gp_theta); %this gets the generalized pareto inverse at evenly spaced quantiles.  The adjustment in the argument is so that we don't try to call the quantile "1".  I pull all quantiles down half a step.
